...
- **Only `*.repository.ts` files may import `PrismaService`, `@prisma/client` or the generated client (`src/generated/prisma`).** Every repository method takes `ownerId` as its first argument, and every query filters on it. The only exemptions are the seed script and the Prisma wiring (`src/prisma/prisma.service.ts`, `src/prisma/prisma.module.ts`); ESLint enforces this.

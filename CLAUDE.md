...
- **Only `*.repository.ts` files may import `PrismaService` or `@prisma/client`.** Every repository method takes `ownerId` as its first argument, and every query filters on it. The seed script is the one exemption.
